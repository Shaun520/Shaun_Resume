...
/**
 * POST /api/resumes/:id/migrate — 简历一键迁移到新模板
 *
 * 请求体：{ targetTemplateId: string }
 * 响应：{ resume, migration: { from, to, preservedFields, migratedAt } }
 *
 * 保留机制：源模板支持但目标模板不支持的 section 对应字段保留在 content 中，
 * 切换回原模板或支持该字段的模板时可恢复。
 */
router.post('/:id/migrate', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const parsed = migrateResumeSchema.safeParse(req.body)
    if (!parsed.success) {
      throw new AppError(400, parsed.error.errors[0]?.message ?? '迁移参数无效')
    }
    const id = req.params.id as string
    const result = await migrateResume(id, req.user!.id, parsed.data.targetTemplateId)
    res.json(result)
  } catch (err) {
    next(err)
  }
