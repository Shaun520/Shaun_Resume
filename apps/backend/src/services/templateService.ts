...
export async function submitTemplate(params: SubmitTemplateParams): Promise<SubmitTemplateResult> {
  ...
  // 存储文件
  const fileUrl = await templateStorageService.saveSubmissionFile(userId, file)
  const thumbnailUrl = await templateStorageService.saveSubmissionThumbnail(userId, thumbnail)
