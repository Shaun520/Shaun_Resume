...
export const resumeService = {
   ...
   * 简历一键迁移到新模板
   * 保留原简历内容，将简历关联到目标模板；
   * 目标模板不支持的字段保留在 content 中，切换回原模板时可恢复
   */
  async migrate(id: string, targetTemplateId: string): Promise<MigrateResumeResponse> {
    const payload: MigrateResumeRequest = { targetTemplateId }
    const res = await apiClient.post<MigrateResumeResponse>(`/resumes/${id}/migrate`, payload)
    return res.data
  },
}
