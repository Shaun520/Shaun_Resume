...
interface ResumeCardProps {
  ...
  onMigrate: (resume: ResumeListItem) => void
}

/** 简历卡片组件：缩略图、标题、编辑时间、操作按钮 */
export default function ResumeCard({ resume, onEdit, onDelete, onMigrate }: ResumeCardProps) {
        ...
        <div className="flex items-center justify-between pt-3 border-t border-warm-100">
          ...
          <nav className="flex gap-1.5" aria-label="简历操作">
            <button
              ...
              aria-label={`迁移简历 ${resume.title} 到新模板`}
              title="迁移模板"
            >
